<template>
  <div>
    <h2 id="page-heading" data-cy="SolicitudHeading">
      <span v-text="$t('archeApp.solicitud.home.title')" id="solicitud-heading">Solicituds</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('archeApp.solicitud.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'SolicitudCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('archeApp.solicitud.home.createLabel')"> Create a new Solicitud </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && solicituds && solicituds.length === 0">
      <span v-text="$t('archeApp.solicitud.home.notFound')">No solicituds found</span>
    </div>
          ...
          <tr v-for="solicitud in solicituds" :key="solicitud.id" data-cy="entityTable">
            ...
            <td class="text-right">
              <div class="btn-group">
                ...
                <b-button
                  v-on:click="prepareRemove(solicitud)"
                  variant="danger"
                  class="btn btn-sm"
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="archeApp.solicitud.delete.question" data-cy="solicitudDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-solicitud-heading" v-text="$t('archeApp.solicitud.delete.question', { id: removeId })">
          Are you sure you want to delete this Solicitud?
        </p>
      </div>
